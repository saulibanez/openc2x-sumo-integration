...
int GpsService::getGpsData(gps_data_t* gpsdata) {
	while (1) {
		...
		if (gpsdata->set && gpsdata->fix.status >= 1) {
			break;
		}
	}
	return 0;				//success
...
void GpsService::receiveData() {
	while (1) {
		...
		if (std::isnan(currentTime) || currentTime == mLastTime) {
		    continue; // skip if time is NaN or no time progressed
		}
...
// INIT SAUL
void GpsService::simulateFromSumo(const boost::system::error_code& error)
{
    if (error) {
        std::cout << "[GpsService] Timer cancelled or error\n";
        return;
    }

	m_sumoInterface->simulationStepWithV2I(mLogger);

	if (m_sumoInterface->hasPendingWarning()) {
		m_triggerSender->send("TRIGGER", "RED_LIGHT");
		std::cout << "[V2I] Enviada alerta RED_LIGHT a DENM (puerto 1111)\n";

		// Descomentar (junto con LDM (ldm.cpp) - METRIC_DENM_RCV) si se quiere revisar envio de mensajes V2I
		// static long seq = 0;
		// seq++;
		// mLogger->logInfo("METRIC_TRIGGER_SEND " + std::to_string(seq) + " " + std::to_string(Utils::currentTimeMs()));

		m_sumoInterface->clearPendingWarning();
	}

    if (!m_sumoInterface->isSimulationRunning()) {
        std::cout << "[GpsService] SUMO simulation ended, stopping GPS service." << std::endl;
		exit(0);
        // return;
    }

    auto ids = m_sumoInterface->getAllVehicleIDs();
    if (ids.empty()) {
        std::cout << "[GpsService] No vehicles available in SUMO." << std::endl;
    }

    for (const std::string& id : ids) {
        double lat = 0, lon = 0, alt = 0;

        if (!m_sumoInterface->getVehiclePosition(id, lat, lon, alt)) {
            std::cout << "[GpsService] Could not get position for vehicle " << id << std::endl;
            continue;
        }

        std::cout << "[GPS] " << id << std::endl;

		gpsPackage::GPS buffer;
		buffer.set_latitude(lat);
		buffer.set_longitude(lon);
		buffer.set_altitude(alt);
		buffer.set_epx(0);
		buffer.set_epy(0);
		buffer.set_time(Utils::currentTime());
		buffer.set_online(0);
		int vehIdNum = std::stoi(id.substr(3));
		buffer.set_satellites(vehIdNum);
		sendToServices(buffer);
    }
    mTimer->expires_from_now(boost::posix_time::millisec(500));
    mTimer->async_wait(boost::bind(&GpsService::simulateFromSumo, 
                                   this, 
...
void GpsService::init() {
	if (!mConfig.mSimulateData) {	//use real GPS data
		...
		std::cout << "-----------------------------Config Mode: " << mConfig.mMode << " --------------------------" << std::endl;
		if(mConfig.mMode == 0) {
		...
		} else if (mConfig.mMode == 2) { // INIT SAUL 
			m_sumoInterface = new SumoInterface("127.0.0.1", 9999);
			if (!m_sumoInterface->connectToSumo()) {
				mLogger->logError("Could not connect to SUMO via TraCI");
				exit(1);
			}
			if (!m_sumoInterface->loadVehicleIDFromSumoConfig(mConfig.mSimConFile)) {
				std::cout<< mConfig.mSimConFile << std::endl;
				mLogger->logError("No se pudo leer el ID del vehículo desde el archivo SUMO");
				exit(1);
			}
			mTimer = new boost::asio::deadline_timer(mIoService, boost::posix_time::millisec(500));
			mTimer->async_wait(boost::bind(&GpsService::simulateFromSumo, this, boost::asio::placeholders::error));
		} // END SAUL
		mIoService.run();
	}
}
