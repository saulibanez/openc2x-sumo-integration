...
DCC::DCC(bool setUpWlan) : mStrand(mIoService) {
	...
	mTimerMeasureChannel = new boost::asio::deadline_timer(mIoService, boost::posix_time::millisec((int)(mConfig.DCC_measure_interval_Tm*1000)));	//1000
	mTimerMeasurePktStats = new boost::asio::deadline_timer(mIoService, boost::posix_time::millisec((int)(mConfig.DCC_collect_pkt_flush_stats*1000)));	//1000
	mTimerStateUpdate = new boost::asio::deadline_timer(mIoService, boost::posix_time::millisec((int)(mConfig.NDL_minDccSampling*1000)));	//1000
	mTimerDccInfo = new boost::asio::deadline_timer(mIoService, boost::posix_time::millisec((int)(mConfig.dccInfoInterval)));

	for (Channels::t_access_category accessCategory : mAccessCategories) {	//create and map the timers for the four ACs
		mTimerAddToken.insert(make_pair(accessCategory, new boost::asio::deadline_timer(mIoService, boost::posix_time::millisec((int)(currentTokenInterval(accessCategory)*1000.00)))));
...
void DCC::measureChannel(const boost::system::error_code& ec) {
	...
	mTimerMeasureChannel->expires_at(mTimerMeasureChannel->expires_at() + boost::posix_time::milliseconds((int)(mConfig.DCC_measure_interval_Tm*1000)));	//1s
	mTimerMeasureChannel->async_wait(mStrand.wrap(boost::bind(&DCC::measureChannel, this, boost::asio::placeholders::error)));
}
...
void DCC::measurePktStats(const boost::system::error_code& ec) {
	...
	mTimerMeasurePktStats->expires_at(mTimerMeasurePktStats->expires_at() + boost::posix_time::milliseconds((int)(mConfig.DCC_collect_pkt_flush_stats*1000)));	//1s
	mTimerMeasurePktStats->async_wait(mStrand.wrap(boost::bind(&DCC::measurePktStats, this, boost::asio::placeholders::error)));
}
...
void DCC::updateState(const boost::system::error_code& ec) {	//TODO: adjust to more than 1 active state
	...
	mTimerStateUpdate->expires_at(mTimerStateUpdate->expires_at() + boost::posix_time::milliseconds((int)(mConfig.NDL_minDccSampling*1000)));	//1s
	mTimerStateUpdate->async_wait(mStrand.wrap(boost::bind(&DCC::updateState, this, boost::asio::placeholders::error)));
}
...
void DCC::addToken(const boost::system::error_code& ec, Channels::t_access_category ac) {
	...
	mTimerAddToken[ac]->expires_at(mLastTokenAt[ac] + boost::posix_time::milliseconds((int)(currentTokenInterval(ac)*1000.00)));	//fixed time interval depending on state and AC
...
void DCC::rescheduleAddToken(Channels::t_access_category ac) {
	if(mAddedFirstToken[ac]) {
		...
		boost::posix_time::ptime newTime = mLastTokenAt[ac] + boost::posix_time::milliseconds((int)(currentTokenInterval(ac)*1000.00));
