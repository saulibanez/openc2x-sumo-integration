...
inline std::string get_openc2x_path(std::string logBasePath, std::string expName, int expNo){
	return logBasePath + "/" + expName + "/" + std::to_string(expNo) + "/";
}

inline void lookup_section(ptree * pt, uci_section *s){
	struct uci_option *o;
...
inline void lookup_config (ptree * pt, std::string config_name, std::string config_type){
...
struct GlobalConfig {
	...
	void loadConfig(const std::string &application_name) {
		...
		mEthernetDevice = pt.get("common.ethernetDevice", "enp0s3");
