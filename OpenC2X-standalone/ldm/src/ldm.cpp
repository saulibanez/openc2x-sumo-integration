...
void LDM::receiveFromCa() {
	...
	while (1) {
		...
		// SAUL: Descomentar (junto con CAM (caservice.cpp) - METRIC_CAM_SEND) si se quiere revisar envio de mensajes V2V
		// static long seq = 0;
		// seq++;
		// mLogger->logInfo("METRIC_CAM_RECV " + std::to_string(seq) + " " + std::to_string(Utils::currentTimeMs()));
...
void LDM::receiveFromDen() {
	...
	while (1) {
		...
		// SAUL: Descomentar (junto con GPS (GpsService.cpp) - METRIC_TRIGGER_SEND) si se quiere revisar envio de mensajes V2I
		// static long seq = 0;
		// seq++; 
		// mLogger->logInfo("METRIC_DENM_RCV " + std::to_string(seq) + " " + std::to_string(Utils::currentTimeMs()));
