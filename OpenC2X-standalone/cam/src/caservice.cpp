...
void CaService::receiveGpsData() {
	...
	while (1) {
		...
		mLogger->logDebug("Received GPS from veh" + std::to_string(newGps.satellites()) +", with latitude: " + to_string(newGps.latitude()) + ", longitude: " + to_string(newGps.longitude()));
...
void CaService::sendCamInfo(string triggerReason, double delta) {
	...
	// SAUL: Descomentar (junto con LDM (ldm.cpp) - METRIC_CAM_RCV) si se quiere revisar envio de mensajes V2V
	// static long seq = 0;
	// seq++;
	// mLogger->logInfo("METRIC_CAM_SEND " + std::to_string(seq) + " " + std::to_string(Utils::currentTimeMs()));
